...
```
**업데이트 안내(1-3쇄만 해당)**: 클로드 아티팩트의 외부 API 정책 변경으로 날씨와 뉴스를 불러오지 못하게 되었습니다. 다음과 같이 대체해 실행해 주세요.
...
```
지금까지 만들어진 문제를 방금 저장한 가이드라인을 참고해서 점검해 줘.
가이드라인에 맞지 않은 퀴즈와 정답이 있다면 이에 맞게 수정해줘.
```
...
#### 05-2 자동화로 개발 효율 높이기 (study-03-basic - 2단계)
...
#### 05-3 명령어 체이닝을 통해 익히는 선택적 업데이트 전략 (study-03-basic - 3단계)
